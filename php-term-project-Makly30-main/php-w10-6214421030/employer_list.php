  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
      ...
      <div class="container">
                    ...
                    <h2>เมนู</h2>
                    <a href="employer_add.php" class="btn btn-link">เพิ่มพนักงาน</a>
    ...
    while (($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) != NULL) {
        ...
        echo '<td>' . $row['em_fname'].' '. $row['em_lname'] ;
        echo '<td>'. $row['em_position'] . '</td>';
        echo '<td>' . $row['em_phone'] . '</td>';
        echo '<td>' . $row['em_address'] . '</td>';
        echo '<td>';
?>
  <a href="employer_edit.php?em_id=<?php echo $row['em_id'];?>" class="btn btn-warning">แก้ไข</a>
                                <a href="JavaScript:if(confirm('ยืนยันการลบ')==true){window.location='employer_delete.php?em_id=<?php echo $row["em_id"];?>'}" class="btn btn-danger">ลบ</a>
                            <?php
                                    echo '</td>';
                                    echo '</tr>';
                                }
