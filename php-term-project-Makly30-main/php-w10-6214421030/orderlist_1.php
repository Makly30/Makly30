...
?>
  ...
  <div class="container-fluid">
    ...
    include "connectdb.php";
    $o_id= $_GET['o_id'];
    $sql="SELECT order_online.o_id,menu.menu_name,order_online.date_process,menu.price,menu.menu_pic,customer.cus_address,promotion.pro_name,promotion.pro_condition,promotion.pro_discount,order_online.o_addtition,order_online.amount,promotion.pro_name,(menu.price*order_online.amount+order_online.delivery_price+promotion.pro_price*menu.price*order_online.amount) as Totalcost,customer.cus_fname,customer.cus_lname,customer.cus_phone,customer.cus_phone,(menu.price*order_online.amount)as totalreal,order_online.delivery_price,promotion.pro_price,order_online.o_addtition,employer.em_fname,employer.em_lname FROM `order_online` join menu on order_online.menu_id=menu.menu_id join promotion on order_online.pro_id=promotion.pro_id join customer on order_online.cus_id=customer.cus_id join employer on order_online.em_id=employer.em_id  where order_online.o_id=".$o_id.";";
    $result = mysqli_query($conn,$sql);
   $row = mysqli_fetch_array($result, MYSQLI_ASSOC);
...
<a href="orderlistadmin.php?o_id=<?php echo $row['o_id'];?>" class="btn btn-info">กลับไปหน้าสั่งซื้อOnline</a>
<br>
      </div>
