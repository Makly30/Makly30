...
<nav class="navbar  navbar-expand-lg navbar-light bg-light ">
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
      ...
      <!-- <form class="form-inline my-2 my-lg-1" action="result_deliver.php">
      <input class="form-control mr-sm-2" name="search_car" type="search" placeholder="Search for  deliver" aria-label="Search">
...
<div class='container'>
 <h1 class='text-center text-success pt-3'>Search For : " <?php echo $_REQUEST['search_car'];?> "</h1>
<div class="row">
 <?php
include 'connectdb.php';

$sql1="SELECT deliver_choices.d_ch_date,deliver_choices.deli_price,deliver_choices.d_ch_int,deliver_choices.deli_pic,deliver_choices.deli_avtime_to,deliver_choices.deli_avtime_from,user_username pk,p1.province_name ps, p2.province_name pt,deliver_choices.deli_time,deliver_choices.d_ch_active,deliver_choices.deli_start,deliver_choices.deli_stop from deliver_choices join user on deliver_choices.deli_deliver_id=user.user_id  join province as p1 on deliver_choices.deli_start=p1.PROVINCE_ID join province as p2 on deliver_choices.deli_stop=p2.PROVINCE_ID where p2.province_name  LIKE '%".$_REQUEST['search_car']."%' or  p1.province_name  LIKE '%".$_REQUEST['search_car']."%' or  p1.province_name  LIKE '%".$_REQUEST['search_car']."%';";
$result1=mysqli_query($conn,$sql1);
$row1=mysqli_fetch_row($result1);
if($row1!=0){
  $sql="SELECT deliver_choices.d_ch_date,deliver_choices.deli_price,deliver_choices.d_ch_int,deliver_choices.deli_pic,deliver_choices.deli_deliver_id,deliver_choices.deli_avtime_to,deliver_choices.deli_avtime_from,user_username pk,p1.province_name ps, p2.province_name pt,deliver_choices.deli_time,deliver_choices.d_ch_active,deliver_choices.deli_start,deliver_choices.deli_stop from deliver_choices join user on deliver_choices.deli_deliver_id=user.user_id  join province as p1 on deliver_choices.deli_start=p1.PROVINCE_ID join province as p2 on deliver_choices.deli_stop=p2.PROVINCE_ID where p2.province_name  LIKE '%".$_REQUEST['search_car']."%' or  p1.province_name  LIKE '%".$_REQUEST['search_car']."%';";
