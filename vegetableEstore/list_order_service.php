   ...
   <table class="table table-bordered table-striped">
                        <thead>
                            <tr>
                                ...
                                <th>รหัส Service </th>
                                <th>Tracking Code</th>
                                <th>วันเดือนปีที่สั่ง</th>
                            </tr>                
                        </thead>
                        <tbody>
      <?php
      include "connectdb.php";
    $sql="SELECT d.dl_id, s.user_username,dc.d_ch_int, tracking.tr_id,tracking.tr_place,dc.deli_price, d.dl_datetime FROM deliver_list as d join user as s on d.dl_s_id=s.user_id join deliver_choices as dc on d.dl_dc_id=dc.d_ch_int join user as k on d.dl_deli_id=k.user_id join tracking on d.dl_id=tracking.tr_de_list_id where dc.deli_deliver_id='".$objResult['user_id']."' order by d.dl_id;";
    ...
    while (($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) != NULL) {
        ...
        echo '<td class="text-center">' . $row['deli_price']. '</td>';
        echo '<td class="text-center">' . $row['d_ch_int']. '</td>';
        echo '<td class="text-center">' . $row['tr_id']. '</td>';
        ...
        echo '<td>';
?>
  <a href="update_location.php?dl_id=<?php echo $row['dl_id'];?>" class="btn btn-success">Update ที่อยู่</a>
  <a href="list_order_detail.php?dl_id=<?php echo $row['dl_id'];?>" class="btn btn-info">ดูรายละเอียด</a>

                                <?php
                                if ($row['tr_place']==null){
                                  echo '<span class="text-danger text-center"><b>ยังไม่ได้ระบุที่อยู่</b></span>';
                                }
                                ?>
                            <?php
                                    echo '</td>';
